...
<body>
	...
	$Tongsodong = mysql_num_rows(mysql_query("SELECT * FROM dkonline")) or die(mysql_error());
	//Tinh so trang
	...
	$Sotrang = $Tongsodong / $Sodong_trentrang + 1 ;
		...
		if(!isset($_GET['page']));
	//Tao cau truy van
	$sql = "SELECT * FROM dkonline LIMIT " . ($page-1) * $Sodong_trentrang . "," . $Sodong_trentrang;
	...
	if (mysql_num_rows($query)==0)
		...
		echo "<tr bgcolor=LightGoldenRodYellow>";
			echo "<td>Số CMND/CCCD</td>";
			echo "<td>Mã Thủ Tục Đăng Ký</td>";
			...
			echo "<td>ĐC Thường Trú</td>";
			echo "<td>ĐC Tạm Trú</td>";
			echo "<td>Hình Ảnh</td>";
			echo "<td>Cha</td>";
			echo "<td>Mẹ</td>";
			echo "<td>Vợ Chồng</td>";
		...
		while($row = mysql_fetch_array($query)){
			echo "<tr bgcolor=LightGoldenRodYellow>";
				...
				echo "<td>" .$row["DCThuongTru"]. "</td>";
				echo "<td>" .$row["DCTamTru"]. "</td>";
				echo "<td><img src='hinhanh/".$row["HinhAnh"]."' width='120' height='200' /></td>";
				echo "<td>" .$row["Cha"]. "</td>";
				echo "<td>" .$row["Me"]. "</td>";
				echo "<td>" .$row["VoChong"]. "</td>";
			echo "</tr>";
		}
		echo "</table>";
		}
	...
	for ( $page = 1; $page <= $Sotrang; $page ++ ){
		echo "<a href='qldangky.php?page={$page}'>{$page} </a>";
...
<div align="center"><img src="hinhanh/baner.png" width="700" height="200" />
</html>
