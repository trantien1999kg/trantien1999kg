...
<body >
	...
	$Tongsodong = mysql_num_rows(mysql_query("SELECT * FROM datlich")) or die(mysql_error());
	//Tinh so trang
	...
	$Sotrang = $Tongsodong / $Sodong_trentrang + 1 ;
		...
		if(!isset($_GET['page']));
	//Tao cau truy van
	$sql = "SELECT * FROM datlich LIMIT " . ($page-1) * $Sodong_trentrang . "," . $Sodong_trentrang;
	...
	for ( $page = 1; $page <= $Sotrang; $page ++ ){
		echo "<a href='QLDatlich.php?page={$page}'>{$page} </a>";
